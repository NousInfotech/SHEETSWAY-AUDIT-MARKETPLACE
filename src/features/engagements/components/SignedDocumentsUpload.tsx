...
function SignedDocumentsUpload({ engagement }: any) {
  ...
  const handleFileUpload = async (
    ...
    try {
      const { uploadUrl, fileUrl, fileKey } = await getUploadUrl(
        file.name,
        file.type,
        'signed-documents' // Example folder
      );
      ...
      if (docType === 'nda') {
        ...
        setNdaInputFile([]); // Clear the input
      } else {
        setEngagementLetterDocument(uploadedDocData);
        setEngagementLetterInputFile([]); // Clear the input
      }
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const body = {
      nda: {
        fileName: ndaDocument.fileName,
        fileKey: ndaDocument.fileKey
      },
      engagementLetter: {
    ...
    try {
      ...
      // Reset the state to allow for new uploads if needed
      setNdaDocument(null);
      ...
      <form onSubmit={onSubmit} className='space-y-8'>
        ...
        <div>
          ...
          <FileUploader
            ...
            disabled={uploading !== null || !!ndaDocument}
          />
          {/* Conditionally render UploadedFileInfo only after successful upload */}
          {ndaDocument && (
            <UploadedFileInfo
              doc={ndaDocument}
        ...
        <div>
          ...
          <FileUploader
            ...
            onUpload={(files) => handleFileUpload(files[0], 'engagementLetter')}
            maxFiles={1}
            maxSize={5 * 1024 * 1024} // 5MB
            accept={{ 'application/pdf': ['.pdf'] }}
            disabled={uploading !== null || !!engagementLetterDocument}
          />
          {/* Conditionally render UploadedFileInfo only after successful upload */}
