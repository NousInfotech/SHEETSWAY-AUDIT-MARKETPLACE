...
// const populatePpeFiles = async (
//   currentState: LibraryData[]
// ): Promise<LibraryData[]> => {
//   const fileSources = [
...
//   const auditProcedures = newState.find(
//     (lib: LibraryData) => lib.name === '5. Audit Procedures'
//   );
//   if (auditProcedures) {
//     const ppeFolder = auditProcedures.subfolders.find(
//       (sub: Subfolder) => sub.name === 'PPE'
//     );
//     if (ppeFolder) {
...
//   // ***************************************************************************************************************
//   // get real root folders, sub folders and files from server

//   const fetchAllRootFolders = async () => {
//     try {
//       let all_roots = await getRoots(engagement.id);
//       console.log('all the roots for this current engagementId:', all_roots);

//       if (all_roots.length > 0) {
//         const allRootFolders: any[] = [];

//         for (const currentRoot of all_roots) {
//           let allFolders = await getRootFolders(currentRoot.id);
//           let rootfolders = allFolders.filter(
//             (item: any) => item.parentId === null
//             // if parantId not null then it become subfolders
//           );

//           allRootFolders.push(...rootfolders);
//         }
//         console.log('allRootFolders', allRootFolders);
...
//     } catch (error) {
//       console.log(error);
//       return []; // Return fallback in case of error
//     }
//   };

//   const fetchSubfoldersbyParentId = async (parentId: string) => {
//     try {
//       let subFolders = getSubFolders(parentId);
//       return subFolders;
//     } catch (error) {
//       console.log(error);
//     }
